...
//////////////// END OPEN MODAL ///////////////////////////////////////////////////RENDERING///////////
 ...
 <a class="event_item-link " href="#">
    <img class="event_item-image" " src="${images?.[7].url ?images?.[0].url:"Not Found"}" alt="${name}" width="180" height="227" loading="lazy"/>
  ...
  const markup = response.data._embedded.events
    .map(({ images, name, dates, _embedded, priceRanges,url }) => {
      ...
      <div class="container__event_card">
        ...
        <p class="modal__p">${_embedded?.venues[0].city ?_embedded?.venues[0].city.name:name} ${_embedded.venues?.[0].country.name}</p>
        ...
        <p class="modal__p">
         ...
         ${priceRanges?.[0].type} ${priceRanges?.[0].min}-${priceRanges?.[0].max} ${priceRanges?.[0].currency}</p>
        ...
        ${priceRanges?.length>1? priceRanges?.[1].type:'Vip'} ${priceRanges?.length>1? priceRanges?.[1].min:'na stanie'}-${priceRanges?.length>1? priceRanges?.[1].max:'0'} ${priceRanges?.length>1? priceRanges?.[1].currency:''}</p>
        <button class="modal__button">BUY TICKETS</button>
        <div><button id='${_embedded.attractions?.[0].id}' class="modal__more-from">MORE FROM THIS AUTHOR</button>
